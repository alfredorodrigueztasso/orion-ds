  ...
  pageHeader={{
    ...
    actions: (
      <>
        ...
        <Button variant="primary" icon={<Plus size={18} />}>New</Button>
      </>
    ),
  }}
  metrics={{
...
import {
  Home, BarChart3, Users, ShoppingCart, Settings, Bell,
  Plus, Download, TrendingUp, DollarSign,
} from 'lucide-react';
          ...
          { key: 'id', header: 'Order', sortable: true },
          {
            ...
            render: (_, row) => (
              <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-3)' }}>
                <Avatar initials={row.customer.split(' ').map(n => n[0]).join('')} size="sm" />
