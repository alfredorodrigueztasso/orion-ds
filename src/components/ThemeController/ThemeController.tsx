...
export const ThemeController: React.FC<ThemeControllerProps> = ({
  ...
  // Try to use context first, fall back to standalone hook
  let themeState;
  try {
  ...
  const { theme, brand, setTheme, setBrand } = themeState;
  ...
  if (compact) {
    return (
      <div className={className} style={{ display: 'flex', gap: 'var(--spacing-3)', alignItems: 'center', flexWrap: 'wrap' }}>
        {showThemeToggle && (
            ...
            <Alert variant="info" title="Current Settings">
              <div style={{ display: 'flex', gap: 'var(--spacing-2)', flexWrap: 'wrap' }}>
                ...
                <Badge variant="neutral">
                  Radius: {BRAND_CONFIG[brand].radius === '9999px' ? '9999px (pills)' : BRAND_CONFIG[brand].radius}
                </Badge>
              </div>
            </Alert>
          )}
