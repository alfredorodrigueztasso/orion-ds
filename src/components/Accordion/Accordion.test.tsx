...
describe('Accordion', () => {
  ...
  it('works as controlled component', async () => {
    ...
    rerender(<Accordion items={mockItems} expanded={['2']} onChange={onChange} />);
  ...
  it('renders items with icons', () => {
    const itemsWithIcons = [
      { id: '1', title: 'Section 1', content: 'Content 1', icon: <span data-testid="icon">📁</span> },
    ];
  ...
  it('applies variant classes', () => {
    const { rerender } = render(
      <Accordion items={mockItems} variant="bordered" data-testid="accordion" />
    );
    expect(screen.getByTestId('accordion').className).toContain('bordered');

    rerender(<Accordion items={mockItems} variant="separated" data-testid="accordion" />);
