...
describe('Avatar', () => {
  ...
  it('fallbacks to icon when image fails and no initials', () => {
    render(<Avatar src="/invalid.jpg" alt="User" icon={<span data-testid="fallback-icon">👨</span>} />);
    const img = screen.getByRole('img');
  ...
  it('applies interactive class when interactive is true', () => {
    ...
    expect((container.firstChild as HTMLElement).className).toMatch(/interactive/);
  });
  ...
  describe('Display priority', () => {
    ...
    it('prioritizes image over icon', () => {
      render(<Avatar src="/user.jpg" alt="User" icon={<span data-testid="icon">👨</span>} />);
      expect(screen.getByRole('img')).toBeInTheDocument();
      expect(screen.queryByTestId('icon')).not.toBeInTheDocument();
    });

    it('prioritizes initials over icon', () => {
      render(<Avatar initials="JD" icon={<span data-testid="icon">👨</span>} />);
      expect(screen.getByText('JD')).toBeInTheDocument();
      expect(screen.queryByTestId('icon')).not.toBeInTheDocument();
    });
