...
export const Calendar: React.FC<CalendarProps> = (props) => {
  ...
  const [currentMonth, setCurrentMonth] = useState(() => {
    if (mode === 'single' && props.mode !== 'range' && props.mode !== 'multiple' && props.selected) {
  ...
  const getDayClassName = (date: Date): string => {
    ...
    if (mode === 'range') {
      ...
      if (rangeState === 'start') {
        ...
        if (props.mode === 'range' && props.selected?.from && props.selected?.to && isSameDay(props.selected.from, props.selected.to)) {
