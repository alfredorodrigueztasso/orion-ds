...
export const Command: React.FC<CommandProps> & {
  ...
  const contextValue = useMemo<CommandContextValue>(
    ...
    [search, setSearch, filter, activeIndex, setActiveIndex, registerItem, unregisterItem, getVisibleItems, listId]
  );
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent) => {
      ...
      if (e.key === 'ArrowDown') {
        ...
        if (activeId) {
          const el = rootRef.current?.querySelector(`[data-command-id="${activeId}"]`) as HTMLElement;
          el?.click();
        }
      }
    },
...
const CommandEmpty: React.FC<CommandEmptyProps> = ({ children, className }) => {
  ...
  return <div className={emptyClasses} role="status" aria-live="polite">{children}</div>;
};
