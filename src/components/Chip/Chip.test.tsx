...
describe('Chip', () => {
  ...
  it('applies variant classes', () => {
    const { rerender } = render(<Chip variant="success" data-testid="chip">Chip</Chip>);
    expect(screen.getByTestId('chip').className).toContain('success');

    rerender(<Chip variant="error" data-testid="chip">Chip</Chip>);
    expect(screen.getByTestId('chip').className).toContain('error');
  });

  it('applies size classes', () => {
    const { rerender } = render(<Chip size="sm" data-testid="chip">Chip</Chip>);
    expect(screen.getByTestId('chip').className).toContain('sm');

    rerender(<Chip size="lg" data-testid="chip">Chip</Chip>);
    expect(screen.getByTestId('chip').className).toContain('lg');
  });

  it('renders with icon', () => {
    render(
      <Chip icon={<span data-testid="icon">★</span>}>
        Chip
      </Chip>
    );
  ...
  it('does not call onClick when disabled', async () => {
    ...
    render(<Chip onClick={onClick} disabled>Disabled</Chip>);
  ...
  it('applies selected state', () => {
    render(<Chip selected data-testid="chip">Selected</Chip>);
    expect(screen.getByTestId('chip').className).toContain('selected');
  });

  it('applies disabled state', () => {
    render(<Chip disabled data-testid="chip">Disabled</Chip>);
    const chip = screen.getByTestId('chip');
  ...
  it('applies custom className', () => {
    render(<Chip className="custom-class" data-testid="chip">Chip</Chip>);
