  ...
  render: () => {
    ...
    return (
      <ChartContainer config={config} style={{ height: 400 }}>
        ...
        >
          <defs>
            <ChartGradient id="fillDesktop" color="var(--color-desktop)" startOpacity={0.4} endOpacity={0.05} />
            <ChartGradient id="fillMobile" color="var(--color-mobile)" startOpacity={0.25} endOpacity={0.02} />
          </defs>
          <CartesianGrid vertical={false} stroke="var(--border-subtle)" />
          <XAxis
            dataKey="date"
...
export const AreaGradientSingle: Story = {
  render: () => {
    ...
    const singleData = areaData.map((d) => ({ date: d.date, desktop: d.desktop }));
