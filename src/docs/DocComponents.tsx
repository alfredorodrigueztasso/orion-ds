...
export function Callout({ variant = 'info', title, children }: CalloutProps) {
  ...
  return (
    <div style={style}>
      <span style={iconStyle} aria-hidden="true">{cfg.icon}</span>
      <div style={{ flex: 1 }}>
        {title && (
          <div style={{ fontWeight: 600, marginBottom: 'var(--spacing-1)' }}>{title}</div>
        )}
        <div>{children}</div>
      </div>
    </div>
...
export function Step({ number, title, children }: StepProps) {
  ...
  return (
    <div style={row}>
      ...
      <div style={{ flex: 1 }}>
        <div style={{ fontWeight: 600, fontSize: '15px', color: 'var(--text-primary)', marginBottom: 'var(--spacing-1)' }}>
          {title}
        </div>
...
export function BrandSwatch({ color, name }: BrandSwatchProps) {
  ...
  return (
    <div style={wrap}>
      <div style={circle} />
      <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-secondary)', fontFamily: 'monospace' }}>{name}</span>
    </div>
  );
}
  ...
  return (
    <table style={table}>
      ...
      <tbody>
        {rows.map((row, i) => (
          <tr key={i}>
            <td style={codeTd}><code>{row.token}</code></td>
