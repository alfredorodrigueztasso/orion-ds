...
export async function init(args: string[]): Promise<void> {
  ...
  if (yes) {
    ...
    const componentDir = await prompt('Component directory:', DEFAULT_CONFIG.componentDir);
    const brand = await prompt('Brand (orion, red, deepblue, orange):', DEFAULT_CONFIG.brand);
    const mode = await prompt('Mode (display, product, app):', DEFAULT_CONFIG.mode);
