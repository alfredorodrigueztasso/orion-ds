...
const EXTENSIONS = new Set(['.tsx', '.ts', '.jsx', '.js', '.css', '.module.css']);
const IGNORED_DIRS = new Set(['node_modules', 'dist', 'build', '.next', '.nuxt', '.git']);
...
function findFiles(dir: string): Array<{ path: string; content: string }> {
  ...
  if (stat.isFile()) {
    ...
    if (EXTENSIONS.has(ext) || EXTENSIONS.has(path.basename(dir).replace(/^[^.]+/, ''))) {
...
`);
  ...
  if (paths.length === 0) {
    console.error('Error: No paths specified. Usage: npx @orion-ds/validate <paths...>');
    process.exit(1);
  }
  ...
  if (allFiles.length === 0) {
    console.log('No .tsx/.ts/.jsx/.js/.css files found in the specified paths.');
    return;
  }
  ...
  if (result.valid) {
    ...
    console.log(`  ${result.summary.totalErrors} errors, ${result.summary.totalWarnings} warnings`);
