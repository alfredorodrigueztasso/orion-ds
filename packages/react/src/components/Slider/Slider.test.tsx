...
describe('Slider', () => {
  ...
  it('renders different sizes', () => {
    const { container, rerender } = render(<Slider value={50} onChange={() => {}} size="sm" />);
  ...
  it('shows min and max labels', () => {
    render(<Slider value={50} onChange={() => {}} min={0} max={100} showLabels />);
  ...
  it('shows tick marks', () => {
    const { container } = render(<Slider value={50} onChange={() => {}} showTicks />);
    expect(container.querySelector('.ticks')).toBeInTheDocument();
  });

  it('shows custom tick values', () => {
    const { container } = render(
      <Slider value={50} onChange={() => {}} showTicks tickValues={[0, 25, 50, 75, 100]} />,
    );
