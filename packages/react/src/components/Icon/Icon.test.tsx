...
describe('Icon', () => {
  ...
  it('applies size token class for named sizes', () => {
    const { rerender } = render(<Icon icon={Search} size="sm" decorative data-testid="icon" />);
  ...
  it('applies color variant classes', () => {
    ...
    rerender(<Icon icon={Search} color="brand" decorative data-testid="icon" />);
    expect(screen.getByTestId('icon').getAttribute('class')).toMatch(/brand/);

    rerender(<Icon icon={Search} color="error" decorative data-testid="icon" />);
    expect(screen.getByTestId('icon').getAttribute('class')).toMatch(/error/);

    rerender(<Icon icon={Search} color="success" decorative data-testid="icon" />);
  ...
  it('applies disabled class', () => {
    ...
    expect(screen.getByTestId('icon').getAttribute('class')).toMatch(/disabled/);
  });

  it('applies custom className', () => {
    render(<Icon icon={Search} decorative className="custom-icon" data-testid="icon" />);
  ...
  it('merges custom style with size override', () => {
    render(<Icon icon={Search} size={24} style={{ opacity: 0.5 }} decorative data-testid="icon" />);
    const icon = screen.getByTestId('icon');
