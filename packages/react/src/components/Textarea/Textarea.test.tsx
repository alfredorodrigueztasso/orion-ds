...
describe('Textarea', () => {
  ...
  it('hides helper text when error is shown', () => {
    render(
      <Textarea label="Comments" helperText="Add your comments" error="This field is required" />,
    );
    expect(screen.getByText(/This field is required/)).toBeInTheDocument();
  ...
  it('supports value prop (controlled)', () => {
    render(<Textarea label="Controlled" value="Initial value" onChange={() => {}} />);
  ...
  it('supports defaultValue prop (uncontrolled)', () => {
    ...
    const textarea = screen.getByLabelText('Uncontrolled') as HTMLTextAreaElement;
  ...
  it('applies resize none class', () => {
    ...
    expect((container.firstChild as HTMLElement).className).toMatch(/resizeNone/);
  });

  it('applies resize vertical class by default', () => {
    const { container } = render(<Textarea label="Vertical Resize" />);
    expect((container.firstChild as HTMLElement).className).toMatch(/resizeVertical/);
  });

  it('applies resize horizontal class', () => {
    const { container } = render(<Textarea label="Horizontal Resize" resize="horizontal" />);
    expect((container.firstChild as HTMLElement).className).toMatch(/resizeHorizontal/);
  });

  it('applies resize both class', () => {
    const { container } = render(<Textarea label="Both Resize" resize="both" />);
    expect((container.firstChild as HTMLElement).className).toMatch(/resizeBoth/);
  });
  ...
  it('applies custom className', () => {
    const { container } = render(<Textarea label="Description" className="custom-textarea" />);
  ...
  it('links helper text with aria-describedby', () => {
    render(<Textarea label="Description" helperText="Provide details" id="description-textarea" />);
    const textarea = screen.getByLabelText('Description');
    expect(textarea).toHaveAttribute('aria-describedby', 'description-textarea-helper');
    expect(document.getElementById('description-textarea-helper')).toHaveTextContent(
      'Provide details',
    );
  });

  it('links error message with aria-describedby', () => {
    render(<Textarea label="Description" error="Required" id="description-textarea" />);
    const textarea = screen.getByLabelText('Description');
    expect(textarea).toHaveAttribute('aria-describedby', 'description-textarea-error');
    expect(document.getElementById('description-textarea-error')).toHaveTextContent('Required');
  });
  ...
  describe('Character Counter', () => {
    ...
    it('counter shows warning state at 90%', async () => {
      const user = userEvent.setup();
      const { container } = render(<Textarea label="Bio" maxLength={10} showCounter />);
      const textarea = screen.getByLabelText('Bio');
    ...
    it('tracks character count for controlled component', () => {
      const { rerender } = render(
        <Textarea label="Bio" value="Initial" maxLength={50} showCounter onChange={() => {}} />,
      );
    ...
    it('tracks character count for uncontrolled component with defaultValue', () => {
      render(<Textarea label="Bio" defaultValue="Hello world" maxLength={50} showCounter />);
  ...
  describe('Resize Behavior', () => {
    ...
    it('applies resize horizontal', () => {
      const { container } = render(<Textarea label="Horizontal" resize="horizontal" />);
      expect((container.firstChild as HTMLElement).className).toMatch(/resizeHorizontal/);
    });
  ...
  describe('All States', () => {
    ...
    it('renders with value', () => {
      render(<Textarea label="With Value" value="Some text" onChange={() => {}} />);
      const textarea = screen.getByLabelText('With Value') as HTMLTextAreaElement;
    ...
    it('renders with helper text', () => {
      render(<Textarea label="With Helper" helperText="Additional information" />);
