...
describe('Stepper', () => {
  ...
  it('marks correct step as current', () => {
    ...
    expect(steps[1].closest('[aria-current]')).toHaveAttribute('aria-current', 'step');
  });
  ...
  it('renders error state', () => {
    const stepsWithError = [
      { id: '1', title: 'Step 1', error: true, errorMessage: 'Something went wrong' },
    ];
  ...
  it('does not trigger click on disabled step', () => {
    ...
    const onStepClick = vi.fn();
    render(<Stepper steps={stepsWithDisabled} clickable onStepClick={onStepClick} />);
  ...
  it('renders different sizes', () => {
    const { container, rerender } = render(<Stepper steps={mockSteps} size="sm" />);
  ...
  it('hides connectors when showConnectors is false', () => {
    const { container } = render(<Stepper steps={mockSteps} showConnectors={false} />);
