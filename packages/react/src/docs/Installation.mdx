...
<Callout variant="info">
  <code>@orion-ds/core</code> is a peer dependency required for design tokens.
</Callout>
...
This bundles both design tokens and component styles.

**Separate imports (if you need granular control):**
...
<Callout variant="warning" title="Missing CSS = unstyled components">
  You must import the CSS file — without it, components will render without any Orion styling.
</Callout>

### ThemeProvider Setup

Wrap your app root with `ThemeProvider`. This is **required** — it manages global theme/brand state and loads fonts automatically.
...
<Steps>
  <Step number={1} title="Initialize your project">
    Run <code>npx @orion-ds/cli init</code> — creates an <code>orion.json</code> config and installs <code>@orion-ds/core</code>.
  </Step>
  <Step number={2} title="Add the components you need">
    Run <code>npx @orion-ds/cli add button card modal</code> — copies component source files, resolves dependencies, transforms imports.
  </Step>
</Steps>

After init, import the token CSS once:
...
<Callout variant="info" title="TypeScript errors?">
  Both packages include type declarations. Make sure <code>@orion-ds/core</code> is installed alongside <code>@orion-ds/react</code>.
</Callout>
